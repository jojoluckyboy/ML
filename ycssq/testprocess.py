...
def match_query(n, lr, queue):
    ...
    partial_match_lr = partial(match_lr, lr=listresult, qresult=listMatch, listduplic=queue)
...
if __name__ == '__main__':
    ...
    c = dict(Counter(dic_list))
    c.items()
    dic_result = {}
    dic_result = sorted(c.items(), key=lambda x: x[1], reverse=False)
    # dic_result =c..

    # for item in  Counter(dic_list).items():
    #     Counter(dic_list).items().
