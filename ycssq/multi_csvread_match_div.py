...
if __name__ == '__main__':
    ...
    reader = pd.read_csv('E:\\data\\twentyone.csv', iterator=True, sep='\s+', dtype=np.int32, chunksize=1000000)
    for chunk in reader:
        df = chunk
        listresult=np.array(df).tolist()

        for i in range(1, 11):
            q_list = manager.list()
            x = len(listresult)*i/10
            k = len(listresult)*(i-1)/10
            ...
            origin = sys.stdout
            f = open('E:/file21.txt', 'a')
            sys.stdout = f
            for item in range(0,len(dic_result)):

                print ("二十一元匹配模式"+str(dic_result[item][0])+"结果正确次数为："+str(dic_result[item][1]))
